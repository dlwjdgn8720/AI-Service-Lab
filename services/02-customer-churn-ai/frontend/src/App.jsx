...
const FIELDS = [
  ...
  { key: "Under 30", label: "30세 미만 여부", type: "select", options: ["No", "Yes"] },
  { key: "Senior Citizen", label: "고령자 여부", type: "select", options: ["No", "Yes"] },
  { key: "Married", label: "결혼 여부", type: "select", options: ["No", "Yes"] },
  { key: "Dependents", label: "부양가족 여부", type: "select", options: ["No", "Yes"] },
  { key: "Tenure in Months", label: "이용 기간 (개월)", type: "number" },
  { key: "Phone Service", label: "전화 서비스 이용", type: "select", options: ["Yes", "No"] },
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      FIELDS.forEach((field) => {
        ...
        visibleValues[field.key] = field.type === "number" ? Number(value) : value;
      });
